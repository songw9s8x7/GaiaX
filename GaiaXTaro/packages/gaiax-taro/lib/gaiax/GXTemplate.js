...
class GXTemplateContext {
    constructor(templateItem, templateInfo) {
...
class GXViewTreeCreator {
    ...
    build(context) {
        ...
        // style={root}
        return (React.createElement(View, null, this.createViewByLayer(context, context.templateInfo.layer, {
            nodeStyle: {},
            nodeCss: {},
        })));
    }
    createViewStyleByCss(context, layer, nodeCss, parentNodeInfo) {
        let nodeStyle = {
            display: 'flex',
            position: "relative",
            direction: 'inherit',
    ...
    createViewByLayer(context, layer, parentNodeInfo, visualNodeInfo = {}) {
        var _a;
        // 获取原始节点样式
        const nodeRawCss = context.templateInfo.css["#" + layer.id] || context.templateInfo.css["." + layer.id];
        // 获取数据绑定
        const nodeData = (_a = context.templateInfo.data["data"]) === null || _a === void 0 ? void 0 : _a[layer.id];
        // 获取事件绑定
        ...
        switch (layer.type) {
            case 'gaia-template':
                if (layer['sub-type'] == 'custom') {
                    ...
                    if (nestTemplateInfo != null && nestTemplateInfo != undefined) {
                        ...
                        return React.createElement(View, { style: nodeStyle, key: layer.id });
                    }
                }
                else {
                    // 普通层级
                    if (layer.layers != null && layer.layers != undefined) {
                        ...
                        return React.createElement(View, { style: nodeStyle, key: layer.id }, childArray);
                    }
                    else {
                        return React.createElement(View, { style: nodeStyle, key: layer.id });
                    }
            ...
            case 'view':
                ...
                return React.createElement(View, { style: nodeStyle, key: layer.id });
            case 'text':
                return React.createElement(Text, { style: nodeStyle, key: layer.id }, dataResult);
            case 'richtext':
                return React.createElement(Text, { style: nodeStyle, key: layer.id }, dataResult);
            case 'iconfont':
                return React.createElement(Text, { style: nodeStyle, key: layer.id }, dataResult);
            ...
            case 'scroll':
                return React.createElement(View, { style: nodeStyle, key: layer.id });
            case 'image':
                return React.createElement(Image, { style: nodeStyle, key: layer.id, src: dataResult });
            default:
                // 不会走到
                return React.createElement(View, { style: nodeStyle, key: layer.id });
        }
    }
    updateViewStyleByCss(context, nodeStyle, layer, nodeCss, parentNodeInfo) {
        // Layout
...
class GXTemplateEngine {
    constructor() {
        this.viewTreeCreator = new GXViewTreeCreator();
        this.dataSource = new GXTemplateDataSource();
    }
    createView(templateItem) {
